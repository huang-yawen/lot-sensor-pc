...
const quickGuide = [
  ...
  { title: '联动控制', tab: 'controlmode', description: '自动模式下用一套统一的规则库自动启停水泵和加热，9 条规则各自独立开关，可以任意组合勾选，不再局限于固定套路。', items: ['水泵常开、加热滞回带通断，是最基础的两条推荐规则', '温度/流量/压力单层规则，以及双温度、温度+流量、压力+流量、温度+压力四条融合规则均可独立勾选', '同一执行器本轮如有多条规则结论矛盾，“关”优先于“开”（fail-safe）'] },
...
const configHelp = [
  ...
  { group: '联动控制', key: 'LINKAGE_RULES', description: '"自动模式"下的正常状况联动统一规则库，9 条规则各自独立开关，可任意组合勾选，不再局限于固定套路。同一执行器本轮如有多条规则结论矛盾，"关"优先于"开"（fail-safe）。', example: 'pumpAlwaysOn: true, heaterHysteresis: true, heaterHysteresisValue: 1', notice: '建议在"联动控制"页可视化修改。' },
  ...
  { group: 'PID恒温', key: 'PID_HEATING', description: '加热模块只有开关量，用时间比例控制模拟 PWM：固定周期 windowMs 内按 PID 算出的占空比决定加热开多久；只接管加热，与 LINKAGE_RULES 的加热下发互斥。', example: 'kp: 20, ki: 0.5, kd: 5, windowMs: 10000', notice: '指令中心 target_temperature 配置了就优先用指令中心的目标温度。' },

...
const quickGuide = [
  ...
  { title: '联动控制', tab: 'controlmode', description: '自动模式下用一套统一的规则库自动启停水泵和加热，规则清单里的规则各自独立开关，可以任意组合勾选，不再局限于固定套路。加热用哪套控制算法是例外，不在这份清单里。', items: ['水泵常开是最基础的推荐规则（不要求已经有流量才能开泵，只看有没有故障和传感器是否异常，冷启动也能正常开泵）；温度/流量/压力单层，以及双温度、温度+流量、压力+流量、温度+压力四条融合规则均可独立勾选', '加热滞回带通断 / PID 恒温是“设备设置/指令配置”页面两个各自独立的开关，两个都开时 PID 优先；现场只想留一种，直接在数据库删掉另一个的指令项，不影响系统运行', '同一执行器本轮如有多条规则结论矛盾，“关”优先于“开”（fail-safe）'] },
  { title: '故障状态', tab: 'fault', description: '干烧/管道堵塞/水泵故障/水泵空转/出水口堵塞共 5 种硬故障，触发后保存故障前快照、强制关闭水泵和加热、复位按钮自动拨到"开"并锁定指令页面其他开关（阈值/参数类指令项不锁，故障期间可以继续调整）。', items: ['5 种硬故障各自独立开关，同时触发时按优先级只显示最高的一个', '人工修复设备后把复位按钮拨回"关"，系统按快照恢复全部开关和参数（含目标温度、Kp/Ki/Kd、各类阈值）、按快照重启执行器', '与“安全联锁”相互独立、都全程生效'] },
  { title: '定量停机', tab: 'quantity', description: '累计流量达到设定值后自动停机，完成定量换热。', items: ['设定定量值（如 500L）', '达到目标关闭水泵和加热', '总流量仅做停机判定'] },
  { title: '需要计算的数据', tab: 'computed', description: '阻力系数、流速、换热效率、液位等工程指标，在首页专用板块展示；同一页面底部还能开关首页"水泵/加热累计运行时长、本次已运行时长"的显示。每个指标的公式和单位见"计算数据"配置页里各项说明。', items: ['阻力系数 K = 泵出口压力 ÷ 流量²（通畅度代理指标）、压力陡降速率 dP/dt、温度变化率 dT/dt', '换热效率 η = 水带走的热功率 ÷ 加热额定电功率 ×100%、热平衡（电功率去向 W 分解）、流量-压力曲线斜率', '累计流量、平均流速 v=Q/A、平均温度（图表见历史图表页面）'] },

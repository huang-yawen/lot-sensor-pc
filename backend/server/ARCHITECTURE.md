...
| 智能判定服务地址 / 请求体字段名 / 结论字段路径 | `service/intelligent/config.js` 的 `INTELLIGENT_JUDGMENT`（13 个键，手动+自动两种模式共用）。只支持 POST JSON→同步 JSON 这一种形态；现场是异步轮询/表单/纯文本时改 `service/intelligent/intelligentJudgment.js` 的 `callService`(发请求) 或 `summarize`(解析响应) |
| 自动判定跑不跑 / 多久提交一次 / 每次取最新几条 | `service/intelligent/config.js` 的 `AUTO_JUDGMENT`；定时逻辑在同目录 `intelligentJudgment.js` 第 4 部分，结果经 WebSocket `auto_judgment` 推给 `src/views/AutoJudgment.vue` |

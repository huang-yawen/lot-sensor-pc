...
| `ALARM_RULES` | 页面自定义阈值告警（可选自动联锁 `action`） | `alarm/evaluateRules.js`、`app.js`、mqtt handlers、errorHistory | `enabled` 总开关、`autoInterlockEnabled`、`rules[]`（`source_table+source_field`/`operator`/`threshold`/`require`/`action`） |

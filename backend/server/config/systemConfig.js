 ...
 * ============================================================================
 *
 * 【分区目录】defaultConfig 很长，按下面的编号分区。要跳到某区，直接搜 "// 编号."
 * （例如搜 "// 10." 跳到安全联锁）。每个键的一句话说明见同目录 CONFIG_CHEATSHEET.md。
 *   1.   场景身份与页面标题          2.   设备模式与字段显示（含 2.1 传感器字段语义槽位）
 *   3.   页面功能开关                4.   累计与滑动统计指标
 *   5.   分页 / 刷新 / 在线判定       6.   MQTT 连接与主题
 *   7.   上报协议兼容                8.   页面术语
 *   9.   HTTP 智能判定适配器         10.  安全联锁（10.5 默认目标温度）
 *   11.  正常状况联动规则            12.  定量停机（12.1 故障状态 / 12.2 PID / 12.4 恒流速）
 *   13.  首页计算展示（13.1 历史图表显隐 / 13.2 运行时长显示）
 *   14.  本地告警与自动联锁规则
 * 分区之后是校验 / 持久化 / 热更新逻辑：搜 "function validate" / "function load" / "function updateConfig"。
 */
